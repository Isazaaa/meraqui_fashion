...
const ProductCard = ({ product }) => {
  ...
  return (
    <Link to={`/producto/${product.id}`} className="no-underline text-inherit block h-full">
      ...
      <article className="flex flex-col h-full overflow-hidden group p-1"> {/* p-1 para un margen visual más sutil */}
        {/* Contenedor de imagen con altura fija y object-cover */}
        <div className={`w-full relative overflow-hidden bg-gray-100 flex justify-center items-center rounded-md ${fixedImageHeightClass}`}>
          {product.image ? (
            <img 
              src={product.image} 
              alt={product.name} 
        ...
        <div className="py-2 px-1 text-center flex flex-col flex-grow"> {/* Reducimos py y px aquí también */}
